"""The scheduled jobs themselves, as plain functions.

Separate from the Celery wrappers in tasks.py because APScheduler's Redis
job store keeps a job as an import path and resolves it on load: it needs a
module-level function, not a Celery Task object. Each job pings its own
Healthchecks check (see heartbeat.monitored) whichever way it was started.
"""
...
async def _screen(watchlist: list[dict]) -> tuple[list[str], Counter]:
    ...
    Was a sequential loop with a fresh `asyncio.run()` per symbol â€” one event
...
def run_screener():
    """Screener task â€” runs every 15 min during NSE market hours via Celery beat."""
    # The calendar is the gate, not the beat schedule. Beat narrows the window
    # to 09:20â€“15:15 IST on weekdays, but it knows nothing about the ~dozen NSE
    ...
    if not market_calendar.is_market_open():
        ...
        logger.info("Screener skipped â€” NSE closed (holiday=%s, trading_day=%s); next open %s",
    ...
    logger.info("Screener complete â€” %d signals published; skipped: %s",
...
def square_off_positions():
    """Close every open paper position at market â€” 15:20 IST, via Celery beat.
    ...
    for the internal square-off endpoint rather than doing the closing itself â€”
    ...
    if not market_calendar.is_trading_day():
        logger.info("Square-off skipped â€” not an NSE trading day")
    ...
    try:
        ...
        logger.error("Square-off failed â€” positions may still be open: %s", e)
        return {"error": str(e)}
...
def generate_morning_brief():
    """Pre-market brief â€” runs ~06:30 IST, after the US close and before the
    ...
    async def run() -> tuple[dict, bool]:
        # One event loop, not two â€” `publish` used to run in a second
...
def refresh_zerodha_session():
    """Daily Kite Connect login â€” 06:00 IST via Celery beat.
    ...
    paper-trading endpoint above. On failure, logged loudly and returned â€”
    not re-raised â€” because a stale token in NestJS degrades gracefully (the
